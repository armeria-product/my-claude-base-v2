...
description: >
  ...
  画像の実際の生成は CLAUDE.md §9（Codex `$imagegen`）に従う。実装が要るなら frontend-design / image-to-code へ。
user-invocable: true
---

## リポジトリ運用メモ — 画像の実行は §9 に従う（最優先）

このスキルは「どんな画像を・どう作るか」の指示（アートディレクション）を担当する。実際の画像生成は
**CLAUDE.md §9** に従い Codex 経由で行う:

- `codex exec --yolo '$imagegen <このスキルが組んだ英語プロンプト>. Save to <path> at <W>x<H>'`
- 保存先: dev モードなら `dev/{product}/assets/`、それ以外は `./assets/`。記述的なファイル名（例 `hero-fintech-teal.png`）。
- 生成後 `ls` で存在確認。空 / エラーなら `codex exec --yolo "/status"` で quota 確認。`codex resume` は使わない（毎回ワンショット）。
- **1 呼び出し＝1 枚**。複数セクションはそれぞれ別呼び出しで順次生成する（下の HARD OUTPUT RULE と一致）。
