...
- **Harness check**: Run `node .claude/scripts/validate.mjs`, and include `VERDICT: PASS/FAIL` and the findings count in the report. node must be on PATH (Windows: `/c/Program Files/nodejs`)
...
- Pure docs / comment-only changes with no harness impact
