...
description: >
  ...
  （主に dev/ 配下の制作物）を作るときに使う。ブリーフを読んで方向性を推論し、テンプレ臭のない
...
## リポジトリ運用メモ（最優先・CLAUDE.md との棲み分け）

- **このスキルの守備範囲**: 実際に動く Web UI / サイト / アプリ（主に `dev/{product}/` 配下の制作物）。
- **対象外 = ユーザーが読む資料・スライド**: レポート / 調査結果 / ガイド / スライドは `doc` スキルが担当し、**CLAUDE.md §10**（外部フェッチ禁止・Web フォント / CDN 禁止・オフライン単一 HTML）に従う。本スキルが前提とする React / Next / Tailwind / CDN / Web フォントのスタックを **§10 成果物に持ち込まない**。
- **画像が要るとき**: 装飾 SVG やコード生成アートを手描きせず、**CLAUDE.md §9**（Codex `$imagegen`）で生成する。参考デザイン画像は `imagegen-frontend-web` / `imagegen-frontend-mobile` / `brandkit`、画像ファーストで起こすなら `image-to-code` に渡す。
- **描画非依存の美学ルール（AIバレ集・パレット・レイアウト規律）は §10 の単一 HTML 資料にも流用可**。持ち込めないのはスタック（CDN / Web フォント / React 依存）だけ。
...
**Status:** schema defined here — **no blocks exist yet in this repository** (the directory below is created only when the first block lands). Do not freelance new blocks without following this schema.

### 12.A File Location
```
.claude/skills/frontend-design/blocks/
  hero/
    asymmetric-split.md
