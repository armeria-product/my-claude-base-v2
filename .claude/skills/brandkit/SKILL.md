...
name: brandkit
description: >
  ブランドキット画像を生成するアートディレクション・スキル（画像のみ・コードは書かない）。
  ブランドガイドライン・ボード、ロゴシステム、アイデンティティ・デッキ、ビジュアルワールドの提案を、
  ミニマル / シネマティック / エディトリアル / ダークテック / ラグジュアリー / セキュリティ / ゲーミング /
  開発者ツール / コンシューマアプリ等の系統で作る。意図あるロゴ設計・洗練構図・余白の効いたタイポ・
  強い象徴性・上質モックを重視。
  「ブランドキット作って」「ロゴ案を画像で」「ブランドガイドを画像で」「アイデンティティのボードを」で発動。
...
このスキルは「どんなブランド画像を・どう作るか」の指示（アートディレクション）を担当する。実際の画像生成は
**CLAUDE.md §9** に従い Codex 経由で行う:

- `codex exec --yolo '$imagegen <このスキルが組んだ英語プロンプト>. Save to <path> at <W>x<H>'`
- 保存先: dev モードなら `dev/{product}/assets/`、それ以外は `./assets/`。記述的なファイル名（例 `brandkit-overview-noir.png`）。
- 生成後 `ls` で存在確認。空 / エラーなら `codex exec --yolo "/status"` で quota 確認。`codex resume` は使わない（毎回ワンショット）。
- **1 呼び出し＝1 枚**。ボード / ロゴ / モックはそれぞれ別呼び出しで順次生成し、系統の一貫性を保つ。
- このスキルは画像のみ。
