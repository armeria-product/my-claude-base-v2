...
description: >
  モバイルアプリ向けの「画面コンセプト画像 / フロー画像」を生成するアートディレクション・スキル
  （画像のみ・コードは書かない）。iOS / Android / クロスプラットフォーム想定で、明確な階層・読みやすい文字・
  複数画面の一貫性・抑えたパレット・上品な端末モックフレームを重視する。既定では控えめな iPhone 風モックの
  中に画面を収めつつ、主役はアプリ内容にする。
  「アプリ画面を画像で」「モバイルの UI 案を画像で」「画面フローを画像で見せて」で発動。
  画像の実際の生成は CLAUDE.md §9（Codex `$imagegen`）に従う。
user-invocable: true
---

## リポジトリ運用メモ — 画像の実行は §9 に従う（最優先）

このスキルは「どんな画面を・どう作るか」の指示（アートディレクション）を担当する。実際の画像生成は
**CLAUDE.md §9** に従い Codex 経由で行う:

- `codex exec --yolo '$imagegen <このスキルが組んだ英語プロンプト>. Save to <path> at <W>x<H>'`
- 保存先: dev モードなら `dev/{product}/assets/`、それ以外は `./assets/`。記述的なファイル名（例 `onboarding-01-dark.png`）。
- 生成後 `ls` で存在確認。空 / エラーなら `codex exec --yolo "/status"` で quota 確認。`codex resume` は使わない（毎回ワンショット）。
- **1 呼び出し＝1 枚**。複数画面はそれぞれ別呼び出しで順次生成し、セット内の一貫性を保つ。
- このスキルは画像のみ。コード実装が要る場合は `frontend-design` へ渡す。
