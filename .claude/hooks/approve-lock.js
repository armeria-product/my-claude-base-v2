...
function arm(root, stateDir, lockPath, payload, namedSlug) {
  ...
  const parts = [
    ...
    '常時書き込み可: tasks/（journal・history 含む）・tmp/ と計画フォルダの PLAN.md・deviations.md（scope.json 自身はロック中は書き込み不可）。範囲外への書き込みはフックが拒否し、その意図は plans/' + slug + '/deviations.md に記録して提案に回すこと（実装は再承認後）。',
