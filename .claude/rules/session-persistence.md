...
**Scope-out**: design artifacts under `plans/{slug}/` (PLAN.md / research.md / deviations.md) are not part of the tasks 6 files.
...
Reader = the next Claude session. **This file duplicates nothing**: next actions / holds / user-facing notes live ONLY in the journal report (/save-session step 3 — SessionStart injects the latest report section reliably). What remains here is the per-product resume anchor and the pointer to the report that is "current".
