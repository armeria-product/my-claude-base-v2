...
6. Symlinks/junctions are not resolved — a junction inside the product root that points outside it is not caught. Accepted limitation: writing an annotation only lets you learn a line number, and whoever can write the CODEMAP can already read the real file.
