...
- `planner` and `reviewer` are authority roles: keep `model: fable` and `effort: max` in frontmatter. Their authority allowlist is exactly native `fable | opus`; only after reporting and recording a Fable availability/usage-limit/startup failure may the caller retry with explicit `model: opus` (explicit model input overrides frontmatter). Never silently fallback or lower to sonnet/haiku/inherit/unknown/external clover ids.
