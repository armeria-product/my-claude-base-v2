...
- **Pin threat model (mutation-observation-points, T4.4 fix pass, 2026-08-15)**: validate.mjs's per-section text pins (§14-16 and later additions in that family) are tamper-EVIDENCE against accidental/careless drift, not tamper-PROOF against an adversarial editor — an editor who can rewrite a duty file can equally rewrite validate.mjs itself, since both share one repo and one write path. A narrow negative-invariant guard exists for the cheapest version of that attack (marking pinned duty text inert with a WITHDRAWN/repealed/retired/not-operative marker while leaving the pinned phrase byte-identical elsewhere in the file). Quoted-repeal, free-form rephrase, and other semantic evasions of a pinned phrase are a known, disclosed, and accepted gap (an unwinnable regex arms race) — not something a future pin is expected to close.
...
2. **Behavior-changing edits**: run one dispatch probe designed so the old and new definition would diverge on it, and record the result in the journal/PR. Adopt a probe only after proving it can fail (mutation-style); if no such probe is designable, record `not behaviorally testable: <reason>` instead. No standing probe suite is kept — explorer / planner / debugger / document-author get validate + first-real-use observation only. executor and verifier are cheap to discriminate, so a designed probe runs for them every time:
   - **executor probe**: seed a `tmp/` mini-project with a 5-line bug plus a deliberately-vacuous always-green test, plus one observation point with a consumption-side line whose removal leaves every test green. PASS = the report shows a RED→GREEN proof, flags the vacuous test, and reports the consumption-side green as an M2 finding (mutation-observation-points Phase 2) — not a pass.
