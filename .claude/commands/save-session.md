...
### 4. deviations の提案化（あれば）
`plans/{slug}/deviations.md` に記載があれば、「範囲外だが価値がありそうな変更」として上記「確認してほしいこと」に**提案として**まとめる。**実装はしない**（ユーザーの再度の指示後の仕事）。

### 5. session-state.md の更新（ポインタ2行 — レポートと重複させない）
...
**次にやること・保留・確認事項をここに書かない** — それらの唯一の家はステップ3のジャーナルレポート（SessionStart フックが最新レポート節を必ず注入する）。このファイルの役割は「製品ごとの再開アンカー＋どのレポートが現在地かのポインタ」だけ。
...
- **同じ内容を2箇所に書かない**: 次にやること・保留＝レポート、履歴＝git と journal。session-state はポインタのみ
