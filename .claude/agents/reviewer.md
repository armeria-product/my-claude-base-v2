...
#### 6. Scope Conformance (mandatory when PLAN.md or scope.json exists)
- Locate the PLAN.md/scope.json named by the dispatch. They are review aids, not a write lock.
- Trace every changed file (`git diff --name-only HEAD`) to a declared task and the `allow`/`forbid` surface. A forbidden or untraceable file is HIGH; CRITICAL when it is destructive or changes settings, hooks, CI, or migrations.
- A run with no PLAN.md or scope.json is casual work: this dimension is N/A; say so explicitly.
- Harness changes are expected only when the user's request or later ruling explicitly names the harness/provider adapter as a target. A plan cannot authorize itself.
- Editing scope-defining `allow`/`forbid`/`tasks` or acceptance criteria during implementation is HIGH unless the user accepted the expansion. Appending results and correcting text after a recorded ruling are expected.
...
- [ ] scope conformance: when PLAN.md/scope.json exists, every changed file traces to a declared task and avoids `forbid`; no untraceable additions (Dimension 6 — HIGH when violated)
